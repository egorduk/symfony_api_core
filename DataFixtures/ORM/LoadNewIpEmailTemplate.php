...
class LoadNewIpEmailTemplate implements FixtureInterface, OrderedFixtureInterface
{
    private $templates = [
        // template name, template description, subject, body
        [
            'user.new_ip',
            '<p>Email to be sent when user performs an action from new ip</p>Available template variables:<ul><li><strong>{{ user.firstname }}</strong> - first name </li><li><strong>{{ user.lastname }}</strong> - last name </li><li><strong>{{ ipAddress }}</strong> - IP Address </li><li><strong>{{ user.username }}</strong> - username </li></ul>',
            'Action from new IP',
            <<<EOD
               ...
               <table width="600" align="center" style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; line-height: 20px; background: #fff; border: 1px #dddddd solid;" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                     <td colspan="2" style="padding:5px 20px 5px 20px;">
                        ...
                        <p>Your account {{user.username}} has logged in from different IP ({{ipAddress}}) address than previously. If you are accessing your account from a different location, disable this notification in your Profile settings or ignore this message. </p>
                        <p>Sincerely,<br />Exmarkets</p>
                     </td>
                  </tr>
                  <tr>
                     <td style="border-top: 1px #ededed solid; text-transform: uppercase; font-size: 11px; color:#777; padding: 15px 20px;">             Copyrights © Exmarkets            </td>
